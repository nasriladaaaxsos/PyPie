...
class PyPie:		# declare a class and give it name User
    def __init__(self, id,  name, firstname, vote):
        self.id = id
        self.name = name
...
def allpies():
    Pie_ = Pie.objects.all()
    list = [] 
    for pie in Pie_: 
        NewPyPie = PyPie(pie.id, pie.piename, pie.createdby.firstname, len(pie.Users.all()) )
        list.append(NewPyPie)

    print(list)
    return sorted(list, key=lambda x: x.vote, reverse=False)
    #to make it descening reverse should be True
